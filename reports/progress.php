...
if (!empty($download)) {
    ...
    if ($format === 'csv') {
        block_reports_custom_export_to_csv($headers, $rows, 'progress_report');
    } else {
        block_reports_custom_export_to_spreadsheet(
            ...
            'progress_report',
            $format,
            get_string('progress_report', 'block_reports_custom')
        );
    }
    exit;
}
...
$records = block_reports_custom_get_progress_records($filters, $page * $perpage, $perpage);
